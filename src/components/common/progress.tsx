...
const Progress = React.forwardRef<HTMLDivElement, ProgressProps>(
  ({ className, value, hintText, activeColorClass, ...props }, ref) => (
    <div ref={ref} className={cn("w-full bg-gray-200 rounded-full relative", className)} {...props}>
      <div className={cn("h-full rounded-full absolute", activeColorClass)} style={{"width": `${zeroOrMinimum(value ?? 0, 10)}%`}}></div>
